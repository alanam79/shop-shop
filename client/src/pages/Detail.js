...
function Detail() {
  ...
  // declare cart as its own variable so that we don't have to keep writing state.cart, must be under the first calling of state
  const { cart } = state;
  ...
  useEffect(() => {
    ...
    // check whether we retrieved data from server using the useQuery hook
    else if (data) {
      dispatch({
