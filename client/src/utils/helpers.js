...
export function idbPromise(storeName, method, object) {
  //  the function to open the connection to the database and create the object stores
  return new Promise((resolve, reject) => {
    ...
    // IndexedDB, the .onupgradeneeded() event only runs if the browser notices that the version number in the .open() method has changed since the last time, or if the browser has never connected to the database before and 1 is the new version.
    ...
    request.onsuccess = function (e) {
      ...
      // code to check which value we passed into the function as a method and perform that method on the object store
      switch (method) {
        case "put":
          store.put(object);
          resolve(object);
